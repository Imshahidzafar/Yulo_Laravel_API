...
class VideoController extends Controller
{
    ...
    public function index(Request $request){
        ...
        /*'".asset('default/music-icon.png')."' as sound_image_url*/
            // DB::enableQueryLog();
        $videos = DB::table("videos as v")->select(DB::raw("v.video_id,v.wide as wide,v.sound_id,case when s.image !='' THEN concat('".$soundImgPath."','/images/',s.image) ELSE ifnull( case when INSTR(su.user_dp,'https://') > 0 THEN su.user_dp ELSE concat('".$userDpPath."/',su.user_id,'/small/',su.user_dp) END,'".asset('default/music-icon.png')."') END as sound_image_url,v.user_id,REPLACE(v.description,'&nbsp;',' ') as description,v.title,case when u.user_dp!='' THEN case when INSTR(u.user_dp,'https://') > 0 THEN u.user_dp ELSE concat('".$userDpPath."/',v.user_id,'/small/',u.user_dp) END ELSE '' END as user_dp, case when v.master_video = ''  then concat('".$videoStoragePath."/',v.user_id,'/',video) else concat('".$videoStoragePath."/',v.user_id,'/',master_video) end as video,ifnull(case when thumb='' then '' else concat('".$videoStoragePath."/',v.user_id,'/thumb/',thumb) end,'') as thumb,ifnull(s.title,'') as sound_title,concat('@',u.username) as username,
       ...
       /* $videos = DB::table("videos as v")->select(DB::raw("v.video_id,v.sound_id,'".asset('default/music-icon.png')."' as sound_image_url,v.user_id,v.description,v.title,case when u.user_dp!='' THEN case when INSTR(u.user_dp,'https://') > 0 THEN u.user_dp ELSE concat('".$userDpPath."/',v.user_id,'/small/',u.user_dp) END ELSE '' END as user_dp, concat('".$videoStoragePath."/',v.user_id,'/',video) as video,case when thumb='' then '' else concat('".$videoStoragePath."/',v.user_id,'/thumb/',thumb) end as thumb,ifnull(case when thumb='' then '' else concat('".$videoStoragePath."/',v.user_id,'/thumb/',thumb) end,'') as thumb,ifnull(s.title,'') as sound_title,concat('@',u.username) as username,
        ...
        if($login_id>0){
            $videos->where(function ($query)  use ($login_id) {
                    $query->where("u.privacy_profile","!=","Private")
                    ->orWhere('u.user_id', $login_id);
                });
        }else{
            $videos->where("u.privacy_profile","!=","Private");
        }
        $videos->orderBy("v.video_id","desc");
    ...
    // public function test_uploadVideo(Request $request){
    //     // print_r($request->all());
    //     // exit;
    ...
    //         $filePath = Storage::disk('azure')->put('uploads/', $request->image);


    //         return response()->json(["status" => "success", "msg"=>"File has been uploaded."]);

    //     }
    //         //     if ($request->hasFile('video')) {
    //         //         $file = $request->file('video')->store('upload','azure');
    //         //         return "File uploaded to Azure Blob Storage: $file";
    //         //     return response()->json(["status" => "success",'msg'=>'Video uploaded successfully', "video" => $file,]);
    //         //     }
    //         // return response()->json(["status" => "error", "msg"=>"video not upload."]);

    //     }
    // }
    public function test_uploadVideo(Request $request)
    {
        print_r($request->video);
        ...
        $filePath = Storage::disk('azure')->putFileAs('uploads', $request->video, $fileName);
        echo $filePath;
        exit;
        
        return response()->json(["status" => "success", "msg" => "Video has been uploaded.","data"=>$filePath]);
    }
     public function shahid_test_uploadVideo(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'video' => 'required|mimes:mp4,mov,ogg,qt',
        ]);
        ...
        $fileName = time() . '_' . $request->video->getClientOriginalName();
        $filePath = Storage::disk('public')->putFileAs('uploads', $request->video, $fileName);

...
class RegisterController extends Controller
{
    ...
    {
         $this->middleware('auth:api', ['except' => ['login', 'isEmailExist', 'isMobileExist', 'resendOtp', 'fetchUserInformation', 'updateUserInformation', 'index', 'register', 'checkExistsEmail', 'verifyOtp', 'socialRegister', 'socialLogin', 'firebaseregister']]);
    }
    ...
    {
        $chkRecord = DB::table('users')->where('mobile', $request->mobile)->exists();
        if ($chkRecord) {
            return response()->json(['status' => 'success', 'isMobileExist' => 'Available']);
        } else {
            return response()->json(['status' => 'success', 'isMobileExist' => 'Unavailable']);
        }
    }
    ...
    public function login(Request $request)
    {
        ...
        if (!$validator->passes()) {
            $data = DB::table('users')->where('mobile', $request->mobile)->get()->first();
            if(isset($data->app_token)){
                $token = $this->getAccessTokenBasedOnUserEmail($data->email);
                ...
                if (!(Hash::check($request->password, $data->password))) {
                    ...
                    if ($data->email_verified == 1) {
                        if ($data->active == 1) {
                            ...
                            DB::table('users')->where('user_id', $data->user_id)->update($data2);
    ...
    // public function login(Request $request)
    // {
    //     if (!$request->mobile && !$request->email && !$request->password) {
    //         return response()->json(['status' => 'error', 'msg' => $this->_error_string($validator->errors()->all())]);
    //     } else {
    //         $functions = new Functions();
        ...
        if (!$validator->passes()) {
            ...
            if (auth()->guard('api')->user()) {
                ...
                if ($userRecord) {
                    ...
                    DB::table('users')
                        ->where('user_id', $userRecord->user_id)
                        ->update(['username' => strtolower($request->username), 'fname' => $fname, 'lname' => $lname, 'email' => $request->email, 'mobile' => $request->mobile != null ? $request->mobile : '', 'bio' => strip_tags($request->bio), 'gender' => $request->gender??'m', 'instagram' => $request->instagram, 'facebook' => $request->facebook, 'youtube' => $request->youtube, 'website' => $request->website, 'dob' => date('Y-m-d', strtotime($request->dob))]);
    ...
    public function firebaseregister(Request $data)
    {
        $rules = [
            'email' => 'required|email|unique:users',
            'fname' => 'required',
